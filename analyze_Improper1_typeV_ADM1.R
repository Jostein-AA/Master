...
### Interaction hyperparameter and prior (Precision and mixing parameter)
interaction_hyper = list(prec= list(prior = 'pc.prec', param = c(1, 0.01)), 
                         phi = list(prior = 'pc', param = c(0.5, 0.5)))
#---
...
#Get formula for type V
typeV_formula_first_level <- update(base_formula_first_level,
                                     ~. + f(space.time, 
                                            model = "bym2",
                                            graph = typeIV_prec_first_level,
...
tryCatch_inla <- function(data,
  ...
  tryCatch(
    {
      tmp_ = inla(typeV_formula_first_level, 
                  data = data, 
                  family = "poisson",
                  verbose = T,
                  E = E_it, #E_it
