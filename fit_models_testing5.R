...
typeIV_constraints_first_level = constraints_maker(type = "IV", n = nrow(first_level_admin_map), t = tT)
typeIV_constraints_second_level = constraints_maker(type = "IV", n = nrow(second_level_admin_map), t = tT)
...
typeIV_formula_first_level <- update(base_formula_first_level, ~. + f(space.time, 
                                              model = "generic0",
...
typeIV_formula_second_level <- update(base_formula_second_level, ~. + f(space.time, 
                                                model = "generic0", 
...
improper_typeIV_sc1 <- inla(typeIV_formula_first_level, 
                             data = lambda_sc1.df, 
                             family = "poisson",
                             E = E_it, 
...
print(c("improper_typeIV_sc1 fitted in: ", Sys.time() - ptm_))
ptm_ = Sys.time()

improper_typeIV_sc3 <- inla(typeIV_formula_first_level, 
                             data = lambda_sc3.df, 
                             family = "poisson",
                             E = E_it, 
                             control.compute = list(config = TRUE, # To see constraints later
...
print(paste("improper_typeIV_sc3 fitted in: ", toString(Sys.time() - ptm_), sep = ""))
ptm_ = Sys.time()

improper_typeIV_sc5 <- inla(typeIV_formula_first_level, 
                             data = lambda_sc5.df, 
                             family = "poisson",
                             E = E_it, 
                             control.compute = list(config = TRUE, # To see constraints later
...
print(paste("improper_typeIV_sc5 fitted in: ", toString(Sys.time() - ptm_), sep = ""))
ptm_ = Sys.time()

improper_typeIV_sc7 <- inla(typeIV_formula_first_level, 
                             data = lambda_sc7.df, 
                             family = "poisson",
                             E = E_it, 
                             control.compute = list(config = TRUE, # To see constraints later
...
print(paste("improper_typeIV_sc7 fitted in: ", toString(Sys.time() - ptm_), sep = ""))
ptm_ = Sys.time()

improper_typeIV_sc9 <- inla(typeIV_formula_first_level, 
                             data = lambda_sc9.df, 
                             family = "poisson",
                             E = E_it, 
                             control.compute = list(config = TRUE, # To see constraints later
                                                    cpo = T)) # For model selection
print(paste("improper_typeIV_sc9 fitted in: ", toString(Sys.time() - ptm_), sep = ""))
ptm_ = Sys.time()

improper_typeIV_sc11 <- inla(typeIV_formula_first_level, 
                              data = lambda_sc11.df, 
                              family = "poisson",
                              E = E_it, 
                              control.compute = list(config = TRUE, # To see constraints later
...
print(paste("improper_typeIV_sc11 fitted in: ", toString(Sys.time() - ptm_), sep = ""))
ptm_ = Sys.time()
...
ptm <- Sys.time(); ptm_ = Sys.time()
improper_typeIV_sc2 <- inla(typeIV_formula_second_level, 
                             data = lambda_sc2.df, 
                             family = "poisson",
                             E = E_it, 
                             control.compute = list(config = TRUE, # To see constraints later
...
print(c("improper_typeIV_sc2 fitted in: ", Sys.time() - ptm_))
ptm_ = Sys.time()

improper_typeIV_sc4 <- inla(typeIV_formula_second_level, 
                             data = lambda_sc4.df, 
                             family = "poisson",
                             E = E_it, 
                             control.compute = list(config = TRUE, # To see constraints later
...
print(c("improper_typeIV_sc4 fitted in: ", Sys.time() - ptm_))
ptm_ = Sys.time()

improper_typeIV_sc6 <- inla(typeIV_formula_second_level, 
                             data = lambda_sc6.df, 
                             family = "poisson",
                             E = E_it, 
                             control.compute = list(config = TRUE, # To see constraints later
...
print(c("improper_typeIV_sc6 fitted in: ", Sys.time() - ptm_))
ptm_ = Sys.time()

improper_typeIV_sc8 <- inla(typeIV_formula_second_level, 
                             data = lambda_sc8.df, 
                             family = "poisson",
                             E = E_it, 
                             control.compute = list(config = TRUE, # To see constraints later
...
print(c("improper_typeIV_sc8 fitted in: ", Sys.time() - ptm_))
ptm_ = Sys.time()

improper_typeIV_sc10 <- inla(typeIV_formula_second_level, 
                              data = lambda_sc10.df, 
                              family = "poisson",
                              E = E_it, 
                              control.compute = list(config = TRUE, # To see constraints later
...
print(c("improper_typeIV_sc10 fitted in: ", Sys.time() - ptm_))
ptm_ = Sys.time()

improper_typeIV_sc12 <- inla(typeIV_formula_second_level, 
                              data = lambda_sc12.df, 
                              family = "poisson",
                              E = E_it, 
                              control.compute = list(config = TRUE, # To see constraints later
...
print(c("improper_typeIV_sc12 fitted in: ", Sys.time() - ptm_))
ptm_ = Sys.time()
...
save(improper_typeIV_sc1,
     ...
     improper_typeIV_sc8,
     improper_typeIV_sc9,
     improper_typeIV_sc10,
     improper_typeIV_sc11,
     improper_typeIV_sc12,
     file = "improper1_typeIV_fitted.RData")