...
proper_onlyInt_formula_first_level <- sampled_counts ~ 1 + time_id + 
                                                      f(area_id, 
                                                        model = "besagproper2",
  ...
  tryCatch(
    {
      ...
      inla.setOption(inla.timeout = 600) # Set to 600 sec (10 minutes!)
      
      tmp_ = inla(proper_onlyInt_formula_first_level, 
                  data = data, 
                  family = "poisson",
                  E = E_it, #E_it
      ...
      marginals = sort_proper_fitted(tmp_$marginals.fitted.values, nrow(new_map), tT) 
      cpo = tmp_$cpo$cpo
