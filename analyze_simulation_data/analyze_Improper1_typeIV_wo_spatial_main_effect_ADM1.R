...
typeIV_prec_second_level <- scaled_RW_prec %x% scaled_Besag_prec_first_level

#Get formula for type IV
typeIV_formula_woSpatial_first_level <- update(base_formula_first_level,
                                                ~. + f(space.time, 
                                                       model = "generic0",
  ...
  tryCatch(
    {
      ...
      tmp_ = inla(typeIV_formula_woSpatial_first_level, 
                  data = data, 
                  family = "poisson",
                  E = E_it, #E_it
