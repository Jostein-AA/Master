...
#setwd("/Users/ingebogh/Documents/makemyprior/other_people/jostein/")
...
raw_prior_typeV = list(
  # tree = 's1 = (s2, s3, s4); s2 = (time_id_iid, time_id_struct); s3 = (area_id_iid, area_id_struct); s4 = (space.time_I, space.time_IV)',
  tree = 's1 = (space.time_I, s2, s3, s4); s2 = (time_id_iid, time_id_struct); s3 = (area_id_iid, area_id_struct); s4 = (space.time_II, space.time_III, space.time_IV)',
  ...
  w = list(s1 = list(prior = "dirichlet"),
           ...
           s3 = list(prior = "pc1",
                     param = 0.51),
           s4 = list(prior = "dirichlet"))
)

# raw_prior_typeV = list(
#   tree = 's1 = (space.time_I, s2); s2 = (s3, s4, s5);
#           s3 = (time_id_iid, time_id_struct); s4 = (area_id_iid, area_id_struct);
#           s5 = (space.time_II, space.time_III, space.time_IV)',
#   V = list(s1 = list(prior = "pc",
#                      param = c(1, 0.01))),
#   w = list(s1 = list(prior = "pc0",
#                      param = 0.5),
...
typeV_prior <- make_prior(typeV_formula, lambda_,
                         family = "poisson",
                         prior = raw_prior_typeV)

### Inspecting the prior
plot_prior(typeV_prior)
plot_tree_structure(typeV_prior)


print("Prior made")

saveRDS(typeV_prior, "./typeV/typeV_prior.rds")
